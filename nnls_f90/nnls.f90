...
!
!  ------------------------------------------------------------------
SUBROUTINE nnls (a, m, n, b, x, rnorm, w, indx, mode, nsetp)
!  ------------------------------------------------------------------
!USE precision
IMPLICIT NONE
INTEGER  :: m, n
INTEGER, DIMENSION(n), INTENT(OUT)      :: indx
INTEGER, INTENT(OUT)                    :: mode, nsetp
REAL(8), DIMENSION(m, n), INTENT(IN OUT) :: a
REAL(8), DIMENSION(m), INTENT(IN OUT) ::  b
!f2py hide,depend(a) :: m=shape(a,0)
!f2py hide,depend(a) :: n=shape(a,1)
!f2py intent(inout) :: a
!f2py intent(inout) :: b
...
REAL(8)               :: alpha, asave, cc, factor = 0.01, sm, &
                           ss, t, temp, two = 2.0, unorm, up, wmax,    &
                           zero = 0.0, ztest
!     ------------------------------------------------------------------
mode = 1
IF (m <= 0 .OR. n <= 0) THEN
...
! IMPLICIT NONE
! 
! REAL(8) :: x(100,19), y(100), t0(19), arg, arglimit, noise(100), t, rnorm, &
!              b(19), w(19)
...
! DO i = 1, 100
!   t = 10. * i
!   y(i) = 100. * ( exp(-t/5.) + exp(-t/50.) + exp(-t/500.) ) +  &
!          noise(i) - 0.5
! END DO
! 
