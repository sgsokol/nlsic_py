C     SUBROUTINE NNLSR  (A,MDA,M,N,B,X,RNORM,W,ZZ,INDEX,MODE)
C
C  Algorithm NNLSR: NONNEGATIVE LEAST SQUARES with RESTART
C
...
c
c  Restarting possibility is added by Serguei Sokol in 2011 to allow
c  repetitive calls with almost same constraints and so with almost
c  same p-z partition. It can happen during non linear ls, for example.
c
...
C                 IZ1 = NSETP + 1 = NPP1
C                 IZ2 = N
C                 If on entry NSETP > 0, INDEX() provides
C                 starting p-z partition
...
C     NSETP   If on entry 0 < NSETP <= N, its value is considered
C             as starting value for NSETP and the content of INDEX()
C             is treated as starting p-z partition
C
C     ------------------------------------------------------------------
      SUBROUTINE NNLSR (A,MDA,M,N,B,X,RNORM,W,ZZ,INDEX,MODE,NSETP)
C     ------------------------------------------------------------------
      ...
      double precision ZERO, ZTEST, DUMMY1(1)
...
C
      DO I=1,N
         X(I)=ZERO
      ENDDO
      if (NSETP .le. 0 .or. NSETP .gt. N) then
         NPP1=1
         DO I=1,N
            INDEX(I)=I
         ENDDO
...
c            RNORM=RNORM+B(I)**2
c         enddo
c         write (*,*) '||b||=', sqrt(RNORM)
c        Here we have initial approximation
         IZ2=N
         IZ1=NSETP+1
         NPP1=1
C        solve ls on given p-set to find x[p-set] and put x[z-set]=0
C        Householder transforms
c         write (*, '(A,I2,I2,I2)') 'M,N, NSETP=', M, N, NSETP
         do L=1,NSETP
            J=INDEX(L)
c         write (*, '(A,I2,I2)') 'L,J=', L, J
C           J IS OK TO BE MOVED TO SET P.
            CALL H12 (1,L,L+1,M,A(1,J),1,UP,B,1,1,1)
c         write (*, '(A,I2,A,F8.3)') ('pivot A(',I,',J)=',A(I,J), I=1,M)
c         write (*, '(A,I3)') 'M', M
c         write (*, '(A,I2,A,F8.3)') ('B(',I,')=',B(I), I=1,M)
C           Apply Householder transform to the rest of columns
            DO JZ=L+1,N
               JJ=INDEX(JZ)
c         write (*, '(A,I2,I2)') 'JZ,JJ=', JZ, JJ
               CALL H12 (2,L,L+1,M,A(1,J),1,UP,A(1,JJ),1,MDA,1)
c         write (*, '(A,I2,A,F8.3)') ('A(',I,',JJ)=',A(I,JJ), I=1,M)
            ENDDO
            DO JJ=L+1,M
               A(JJ,J)=ZERO
            ENDDO
         enddo
c         write (*, '(4(A,I2,A,F8.3,X))') ('A(',I,')=',A(I,1), I=1,M*N)
C                                SOLVE THE TRIANGULAR SYSTEM.
C                                STORE THE SOLUTION TEMPORARILY IN ZZ[p-set].
c        copy b to w
         do L=1,N
            W(L)=B(L)
         enddo
c         write (*, '(A,I2,A,F8.3)') ('avant w(',I,')=',w(I), I=1,N)
         DO L=1,NSETP
            IP=NSETP+1-L
            IF (L .ne. 1) THEN
               DO II=1,IP
                  W(II)=W(II)-A(II,JJ)*W(IP+1)
               ENDDO
            ENDIF
            JJ=INDEX(IP)
            W(IP)=W(IP)/A(IP,JJ)
c         write (*, '(A,I2,A,F8.3)') 'pendant zz(',IP,')=',ZZ(IP)
         ENDDO
C        permute ZZ to get X
         DO IP=1,NSETP
            I=INDEX(IP)
            X(I)=W(IP)
         ENDDO
...
c            RNORM=RNORM+B(I)**2
c         enddo
c         write (*,*) 'after ||b||=', sqrt(RNORM)
c         write (*, '(A, F8.3)') ('x=', X(I), I=1,N)
c         write (*, *) 'NSETP, INDEX=', NSETP, INDEX
c        ensure that all x[p-est] are non negative
         do IP=1,NSETP
            J=INDEX(IP)
            if (X(J) .le. 0.) then
c              initial approximation for INDEX was not good enough
c              get back to all zero starting point
C
               do I=1,N
                  X(I)=0.
                  INDEX(I)=I
...
c         write (*, '(A,4(I2X))') 'IZ1,IZ2,NSETP,NPP1=',IZ1,IZ2,NSETP,NPP1
c         write (*, '(A, F8.3)') ('pos(x)=', X(I), I=1,N)
   30 CONTINUE
...
C
  ...
  380       W(J)=ZERO
      ...
      RNORM=sqrt(SM)
c      write (*,'(A,I2)') 'On exit NSETP=', NSETP
      RETURN
C
